...
inline float process_temp(const string &temp,
                          const map<string, float> &lookup) {
  try {
    return stof(temp);
  } catch (...) {
  }
  string upper_temp = temp;
  for (auto &c : upper_temp)
...
inline void generate_contours(const string &filename, float layer_height,
                              float scale, float base_offset,
                              vector<vector<FaceQueue>> &face_qs_out,
  ...
  int num_slices =
      static_cast<int>(ceil((z_max - base_offset) * scale / layer_height));
  ...
  for (int i = 0; i < num_slices; ++i) {
    ...
    for (size_t face_num = 0; face_num < raw_faces.size(); ++face_num) {
      ...
      for (int idx : face_indices)
        current_verts.push_back(vertices_out[idx]);
      ...
      if (!lowers.empty() && !uppers.empty()) {
        ...
        for (const auto &low_vert : lowers)
          for (const auto &upp_vert : uppers)
            f_class.add_contour_pts(
                get_intersection(low_vert, upp_vert, Axis::Z, zi));

        FaceQueue &active_fq = layer_fqs.back();
        bool isFqFull = active_fq.insert(f_class);

        if (isFqFull &&
            (!active_fq.store.empty() || (face_num < raw_faces.size() - 1))) {
          FaceQueue extra_face_q;
          for (const auto &f : active_fq.store)
...
inline void generate_gcode(
  ...
  generate_contours(filename, layer_height, scale, base_offset, face_qs,
                    vertices);

  if (feedrate_writing < 0)
    feedrate_writing = static_cast<int>(feedrate) / 2.0f;

  float flow_area = extrusion_multiplier * extrusion_width * layer_height;
  float flowrate = flow_area * feedrate_writing / 60.0f;
  float extrusion_rate = flow_area / (filament_diameter * filament_diameter /
  ...
  cerr << "INFO: The nozzle temperature is set to " << nozzle_temp
       << " degrees celsius" << endl;
  cerr << "INFO: The bed temperature is set to " << bed_temp
       << " degrees celsius" << endl;

  float total_distance = 0, total_extruded = 0;

  auto ftos = [](float f) -> string {
    ostringstream ss;
    ss << f;
    return ss.str();
  };
  ...
  string units_str =
      (units == "in") ? "0 \t\t\t\t\t;use inches" : "1 \t\t\t\t\t;use mm";

  process_gcode_template(template_dir + "header.gcode", "header.tmp",
                         {{"units", units_str},
                          {"feedrate", ftos(feedrate)},
                          {"temperature", ftos(nozzle_temp)},
                          {"bed_temperature", ftos(bed_temp)}});
  process_gcode_template(template_dir + "footer.gcode", "footer.tmp",
                         {{"feedrate", ftos(feedrate)}});

  GCodeWriter g(outfile, filament_diameter, layer_height, "header.tmp",
                "footer.tmp", vertices);
  g.open();
  g.absolute();
  ...
  for (size_t layer_num = 0; layer_num < face_qs.size(); ++layer_num) {
    ...
    for (const auto &contour : layer_qs) {
      ...
      for (size_t i = 0; i < contour.q.size(); ++i) {
        ...
        if (i == 0) {
          ...
          next_pt = (face.contour_points[0] == last_pt)
                        ? face.contour_points[1]
                        : face.contour_points[0];
        }

        float distance = distance_between(next_pt, last_pt);
        total_distance += distance;
        float extrusion_amount = extrusion_rate * distance;
        g.abs_move(next_pt.x, next_pt.y, next_pt.z, false, feedrate_writing,
      ...
      total_distance += distance;
      float extrusion_amount = extrusion_rate * distance;
      g.abs_move(start_pt.x, start_pt.y, start_pt.z, false, feedrate_writing,
                 total_extruded + extrusion_amount);
      total_extruded += extrusion_amount;
    }
    ...
    if (static_cast<int>(layer_num) < num_solid_fill ||
        ...
        misc_infill == "solid") {
      int axis = (layer_num % 2 == 0) ? static_cast<int>(Axis::X)
                                      : static_cast<int>(Axis::Y);
      solid_infill(g, layer_qs, axis, vertices_axis_min(vertices, axis),
                   vertices_axis_max(vertices, axis), extrusion_rate,
                   total_extruded, total_distance, extrusion_width);
    } else if (misc_infill == "cross") {
      criss_cross(g, layer_qs,
                  vertices_axis_min(vertices, static_cast<int>(Axis::X)),
                  ...
                  extrusion_width, gap_between_crosses);
    }
  }
